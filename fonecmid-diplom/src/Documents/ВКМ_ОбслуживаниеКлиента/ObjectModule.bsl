...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	// Проверка договора и определение стоимости часа работы
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
	...
	// Проверка и получение процента за работу сотрудника
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ВКМ_УсловияОплатыСотрудниковСрезПоследних.ПроцентОтРабот
		|ИЗ
		|	РегистрСведений.ВКМ_УсловияОплатыСотрудников.СрезПоследних(&Период, Сотрудник = &Специалист) КАК
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон("Для сотрудника %1 не задан процент от работ", Специалист);
	...
	// Запись данных в регистры выполненных работ
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Движение.СуммаКОплате = ВыборкаДетальныеЗаписи.ЧасыКОплатеКлиенту * СтоимостьЧасаРаботы * ПроцентОтРабот / 100;
